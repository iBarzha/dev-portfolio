...
class ProjectViewSet(viewsets.ReadOnlyModelViewSet):
    """
    ViewSet for Project model with GitHub integration and language filtering

    Provides:
    - List view with basic GitHub stats (filtered by language)
    - Detail view with complete GitHub data
    - Action to refresh GitHub data
    - Filtering by category, status, featured, language
    """

    # Base queryset - will be filtered by language in get_queryset
    ...
    def get_queryset(self):
        ...
        # Filter by language - MOST IMPORTANT
    ...
    def refresh_github_data(self, request):
        ...
        if project_ids:
            ...
            projects = Project.objects.filter(show=True, language=language)
        ...
        return Response({
            'message': f'GitHub data refresh completed for {language} projects',
            'language': language,
    ...
    def categories(self, request):
        """Get available project categories for specific language"""
    ...
    def stats(self, request):
        """Get overall portfolio statistics for specific language"""
        language = request.query_params.get('language', 'en')
        projects = Project.objects.filter(show=True, language=language)
